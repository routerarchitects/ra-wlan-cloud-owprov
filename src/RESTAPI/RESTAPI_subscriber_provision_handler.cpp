...
namespace OpenWifi {
	...
	bool RESTAPI_subscriber_provision_handler::CreateVenueRecord(ProvisionContext &ctx) {
		...
		ctx.signupRecord.venueId = ctx.venueRecord.info.id;
		SignupDB_.UpdateRecord("id", ctx.signupRecord.info.id, ctx.signupRecord);
	...
	bool RESTAPI_subscriber_provision_handler::UpdateInventoryRecord(ProvisionContext &ctx) {
		...
		AddMembership(StorageService()->VenueDB(), &ProvObjects::Venue::devices,
					  ctx.inventoryRecord.venue, ctx.inventoryRecord.info.id);
		SDK::GW::Device::SetOwnerShip(this, ctx.inventoryRecord.serialNumber, ctx.inventoryRecord.entity,
									  ctx.inventoryRecord.venue, ctx.inventoryRecord.subscriber);
		poco_trace(Logger(), fmt::format(
									   "[SUBSCRIBER_PROVISION]: Device with serial number {} "
	...
	bool RESTAPI_subscriber_provision_handler::DeleteVenueRecord(ProvisionContext &ctx) {
		...
		poco_information(Logger(),
							fmt::format("[SUBSCRIBER_PROVISION]: Deleting venue record {}.",
										ctx.venueRecord.info.id));
		if (!VenueDB_.GetRecord("id", ctx.venueRecord.info.id, venueRecord)) {
			poco_error(Logger(),
					   fmt::format("[SUBSCRIBER_PROVISION]: Venue record {} not found.",
								   ctx.venueRecord.info.id));
			NotFound();
			return false;
		}
	...
	void RESTAPI_subscriber_provision_handler::DoDelete() {
		...
		poco_information(Logger(),
							fmt::format("[SUBSCRIBER_PROVISION]: Unprovision request received."));
		...
		ctx.signupRecord.userId = subscriberId;
		poco_information(Logger(),
							fmt::format("[SUBSCRIBER_PROVISION]: Unprovisioning subscriber {}.",
										subscriberId));
		if (!LoadSignupRecord(ctx))
			return;
		poco_information(Logger(),
							fmt::format("[SUBSCRIBER_PROVISION]: Loaded signup record for subscriber {}.",
										subscriberId));
		if (!LoadOperatorRecord(ctx))
			return;
		poco_information(Logger(),
							fmt::format(
								"[SUBSCRIBER_PROVISION]: Loaded operator record for subscriber {}.",
								subscriberId));
		...
		poco_information(Logger(),
							fmt::format(
								"[SUBSCRIBER_PROVISION]: Unlinked inventory record for subscriber {}.",
								subscriberId));
		if (!ctx.signupRecord.venueId.empty()) {
			if (!VenueDB_.GetRecord("id", ctx.signupRecord.venueId, ctx.venueRecord)) {
				NotFound();
				return;
			}
			if (!StopMonitoring(ctx))
				return;
			poco_information(Logger(),
								fmt::format("[SUBSCRIBER_PROVISION]: Stopped monitoring for subscriber "
											"{}.",
											subscriberId));
			if (!DeleteVenueRecord(ctx))
				return;
			poco_information(Logger(),
							 fmt::format("[SUBSCRIBER_PROVISION]: Deleted venue record for subscriber "
										 "{}.",
										 subscriberId));
		}
		poco_information(Logger(),
							fmt::format("[SUBSCRIBER_PROVISION]: Unprovisioning of subscriber {} completed.",
										subscriberId));
		OK();
	}
